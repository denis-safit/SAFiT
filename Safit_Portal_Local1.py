...
# --- 1. CONFIGURAZIONE E STILE (Come v2.4.0) ---
...
# --- 3. FUNZIONI TECNICHE (Migliorate per Precisione Dato) ---
def clean_num(serie):
...
# --- 4. MOTORE DI CALCOLO ATP (Riscritto su campi reali Access) ---
...
def load_and_process():
    try:
        ...
        # Carico Access con i nuovi campi richiesti
        df_acc = smart_load('Avanzamento_access.xlsx', "CODICE")
        stock = {}
        if not df_acc.empty:
            for _, r in df_acc.iterrows():
                art_code = str(r['CODICE']).strip().upper()
                # GIA reale (non SLD_M), INACQ e Avanzamento (TMP+RWI+GRZ)
                stock[art_code] = {
...
if not df_res.empty:
    with st.sidebar:
        ...
        st.write("### Filtra Stato:")
        sel_stati = [s for s in ["DISPONIBILE", "ACQUISTO", "PRODUZIONE", "MANCANTE", "DA PIANIFICARE"] if st.checkbox(s, value=True)]
    ...
    st.sidebar.download_button("📊 Excel", data=to_excel(df_f), file_name=f"Safit_{datetime.now().strftime('%d%m')}.xlsx", use_container_width=True)
    ...
    with c2:
        ...
        st.plotly_chart(px.pie(df_f.groupby('Famiglia')['Qta Residua'].sum().reset_index().head(10), values='Qta Residua', names='Famiglia', hole=0.4, title="Top 10 Famiglie"), use_container_width=True)
