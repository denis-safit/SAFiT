...
st.markdown("""
    ...
    /* ---- PROGRESS BAR ---- */
    .pbar-wrap { background:#fff; border-radius:8px; padding:10px 14px; margin-bottom:10px;
...
# --- 2. GESTIONE UTENTI (IDENTICA ALL'ORIGINALE) ---
@st.cache_data
def load_users():
    file_u = 'utenti.xlsx'
...
# --- 3. FUNZIONI TECNICHE (IDENTICHE ALL'ORIGINALE) ---
...
# --- 4. FUNZIONI HTML GRAFICHE ---
# Costruite con concatenazione di stringhe (NO f-string annidati → nessun HTML rotto)
...
def html_timeline(step_idx):
    # step_idx: 0=Ordinato, 1=In Lavorazione, 2=Pronto, 3=Consegnato
    parts = []
    for i, (lbl, icon) in enumerate(zip(TL_LABELS, TL_ICONS)):
        ...
        if i < len(TL_LABELS) - 1:
            line_cls = "tl-line tl-line-done" if i < step_idx else "tl-line tl-line-pend"
            parts.append('<div class="' + line_cls + '"></div>')
    return '<div class="tl-wrap">' + ''.join(parts) + '</div>'

def html_kpi_summary(cnts, cliente):
    tot   = sum(cnts.values()) or 1
    pct_p = int((cnts['OK'] + cnts['BLU']) / tot * 100)
    segs  = [
        (cnts['OK'],  '#4caf50', '✓'),
        (cnts['BLU'], '#2196f3', '●'),
        (cnts['LAV'], '#ffc107', '⚙'),
    ...
    return (
        ...
        '</div>'
        '<div style="font-size:11px;font-weight:700;color:#555;margin-bottom:4px;">Avanzamento complessivo (' + str(pct_p) + '% pronto)</div>'
        '<div class="sbar">' + sbar + '</div>'
        '</div>'
    )

# --- 5. SIDEBAR (IDENTICA ALL'ORIGINALE) ---
with st.sidebar:
...
# --- 6. LOGICA CENTRALE (IDENTICA ALL'ORIGINALE + grafica) ---
...
if not df_cli.empty:
    ...
    # --- RIEPILOGO KPI (solo quando si vedono tutti i prodotti) ---
    ...
    for art in articoli_view:
        ...
        # Giacenza e lavorazione — STESSO codice dell'originale, non toccato
        ...
        # Barra giacenza articolo (calcolata prima del loop, non consuma st_gia)
            ...
            with st.expander(f"📦 {art} — {desc} | Residuo: {df_art['Qta_Effettiva'].sum():,.0f}"):
                ...
                st.markdown(
                    html_pbar(pct_art, bar_col,
                              "Giacenza: " + f"{qta_pronta:,.0f}" + " / " + f"{qta_tot:,.0f}" + " pz"),
                    unsafe_allow_html=True
                )
                ...
                for r in righe_mostra:
                    ...
                    st.markdown(
                        ...
                        f'<span><b>Consegna:</b> {date_str} | <b>Q.tà:</b> {r["qta"]:,.0f}</span>'
