...
# --- 4. MOTORE DI CALCOLO (LOGICA MASTER 3.2.8 + INTEGRAZIONE OFF) ---
...
def load_and_process():
    try:
        ...
        # --- LOGICA OFF: Recupero date prima del filtro OCI/OCA ---
        ...
        for index, row in df_orders.iterrows():
            ...
            # Recupero data OFF per questo articolo
...
if not df_res.empty:
    ...
    st.markdown(f'<div class="user-info">Utente: <b>{st.session_state.user}</b></div>', unsafe_allow_html=True)
    ...
    st.download_button("Scarica Report Excel", data=to_excel(df_res), file_name="Report_Safit.xlsx")
