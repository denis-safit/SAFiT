...
# --- 1. CONFIGURAZIONE E STILE (RIPRISTINATO 1.4) ---
APP_VERSION = "1.4.1-GIA-Fix"
...
# --- 4. MOTORE DI CARICAMENTO (OTTIMIZZATO PER GIA) ---
def clean_num(serie):
...
def load_all():
    try:
        ...
        if os.path.exists('Avanzamento_access.xlsx'):
            ...
            # --- FIX CRUCIALE: Ricerca precisa colonne ---
            ...
            if c_code:
                ...
                df_t['PROD_V'] = 0
                for f in ['TMP', 'RWI', 'TRS']: # Rimosso LANCIATI per evitare doppioni
...
if not data.empty:
    ...
    for art in df_cli['Art_Key'].unique():
        ...
        # Prendi i valori di stock una sola volta per articolo
