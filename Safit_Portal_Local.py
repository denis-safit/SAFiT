...
# FUNZIONE MODIFICATA PER CORRISPONDENZA ESATTA
...
def load_all():
    try:
        ...
        if os.path.exists('Avanzamento_access.xlsx'):
            ...
            # Utilizzo corrispondenza ESATTA per evitare GIA_FRI
...
if not data.empty:
    ...
    with st.sidebar:
        ...
        if st.session_state["user_type"] == "TUTTI":
            sel_cli_list = sorted(data[c_cli_n].unique().astype(str))
            sel_cli = st.selectbox("Seleziona Cliente:", sel_cli_list)
        else:
    ...
    for art in sorted(df_show['Art_Key'].unique()):
        ...
        with st.expander(f"📦 {art} - {desc_val}"):
            st.markdown(f'<div class="debug-box">Giacenza (GIA): {df_sub["GIA_V"].iloc[0]:,.0f} | In Arrivo: {df_sub["ACQ_V"].iloc[0]:,.0f} | Produzione: {df_sub["PROD_V"].iloc[0]:,.0f}</div>', unsafe_allow_html=True)
