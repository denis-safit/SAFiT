...
# --- 1. CONFIGURAZIONE E STILE (Dalla v2.4) ---
APP_VERSION = "3.2.0-Full-Merge-Final"
...
# --- 4. MOTORE DI CALCOLO (LOGICA 1.4 INTEGRATA IN 2.4) ---
...
def load_and_process():
    try:
        ...
        # Carico Access con i campi della v1.4
        ...
        # Calcolo Copertura (LOGICA SEQUENZIALE V1.4)
        ...
        for art, group in df_orders.groupby(c_art):
            ...
            for _, row in group.iterrows():
                ...
                # Sovrascrittura per OCA (Previsioni) come nella v2.4
...
# --- 5. LOGICA DI ACCESSO (Dalla v2.4) ---
...
# --- 6. DASHBOARD COMPLETA (Dalla v2.4) ---
...
if not df_res.empty:
    with st.sidebar:
        ...
        stati_p = ["DISPONIBILE", "ACQUISTO", "PRODUZIONE", "MANCANTE", "DA PIANIFICARE"]
        sel_stati = [s for s in stati_p if st.checkbox(s, value=True, key=f"ch_{s}")]
    ...
    # Esportazione Excel (Dalla v2.4)
    st.sidebar.download_button("📊 Esporta in Excel", data=to_excel(df_f), file_name=f"Safit_Report_{datetime.now().strftime('%d%m')}.xlsx", use_container_width=True)
